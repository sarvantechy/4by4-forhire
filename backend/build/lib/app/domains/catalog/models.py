...
class Listing(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("listing_type IN ('item','service')", name="ck_listings_listing_type"),
        ...
        Index("ix_listings_store_status", "store_id", "status"),
    )
    ...
    listing_type: Mapped[str] = mapped_column(String(16), nullable=False, default="item")
    ...
    # Composite FK (store_id, owner_user_id) -> stores(id, owner_user_id) is enforced
    # in the database migration; there is no single-column FK on this side.
    ...
    store: Mapped[Store | None] = relationship(
        Store,
        primaryjoin=(
            "and_(foreign(Listing.store_id) == Store.id, "
            "Listing.owner_user_id == Store.owner_user_id)"
        ),
        viewonly=True,
    )
